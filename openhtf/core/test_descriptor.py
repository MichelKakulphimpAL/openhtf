...
class Test(object):
  ...
  def add_additional_handlers(self, *handlers: logging.Handler) -> None:
    """Adds logging handlers that will handle log records produced by the test during execution."""
    self._test_options.additional_handlers.extend(handlers)
...
class TestOptions(object):
  ...
  diagnosers: list of BaseTestDiagnoser subclasses to run after all the
      phases.
  additional_handlers: list of user-defined logging handlers that handle log
      lines generated by the test.
  """
  ...
  additional_handlers = attr.ib(type=List[logging.Handler], factory=list)
